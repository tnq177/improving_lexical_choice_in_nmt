...
class Model(object):
    def __init__(self, config, mode, lex_table_path):
        ...
        self.lex_table = tf.get_variable('lex_table',
                                        dtype=tf.float32,
                                        initializer=tf.constant_initializer(value=lex_table_npy),
                                        shape=[src_vocab_size, trg_vocab_size],
                                        trainable=False)
        ...
        lex_inputs = tf.nn.embedding_lookup(self.lex_table, self.src_inputs)
        ...
        def decoder_output_func(h_t):
            ...
            lex_probs = tf.multiply(tf.reshape(alignments, [batch_size, -1, 1]), lex_inputs)
            lex_probs = tf.reduce_sum(lex_probs, 1)
            return output, lex_probs, alignments
        ...
        attentional_outputs, lexical_probs = decoder.decode(decoder_inputs,
        ...
        def logit_func(att_output, lex_prob):
            ...
            lex_logit = tf.reshape(lex_logit, [-1, trg_vocab_size])

            return nmt_logit + lex_logit

        logits = logit_func(attentional_outputs, lexical_probs)
